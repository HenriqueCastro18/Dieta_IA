  ...
  useEffect(() => {
    if (selectedDate && userId) {
      const loadData = async () => {
        setLoading(true);
        try {
          // @ts-ignore - Ignora erro se o VS Code ainda não reconheceu o método novo no DBService
          ...
          <motion.div key="month" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} style={contentArea}>
            ...
            <div style={calendarGrid}>
              ...
              {Array.from({ length: new Date(currentYear, currentMonth + 1, 0).getDate() }, (_, i) => {
                ...
                const isToday = dKey === todayDate.toLocaleDateString('pt-BR');
                ...
                return (
                  <div key={i} 
                    ...
                    style={{ 
                      ...dayCell, 
                      cursor: 'pointer', 
                      background: isToday ? 'linear-gradient(135deg, #00f2fe 0%, #4facfe 100%)' : 'rgba(255,255,255,0.02)', 
                      color: isToday ? '#000' : '#444',
                      border: isToday ? '1px solid #00f2fe' : '1px solid rgba(255,255,255,0.03)'
                    }}>
                    {dayNum}
                  </div>
                );
          ...
          <motion.div key="meal" initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 1.05, opacity: 0 }} style={contentArea}>
            ...
            <div style={premiumDetailsCard}>
              <header style={cardHeader}>
                ...
                <p style={mealDetailMeta}>{selectedMeal.date}</p>
              </header>

              <div style={summaryHeader}>
                <div style={summaryMainInfo}>
                  <span style={summaryCalValue}>{Math.round(selectedMeal.macros?.calories || 0)}</span>
...
const weekDayHeader: React.CSSProperties = { textAlign: 'center', fontSize: '10px', color: '#222', fontWeight: '900', paddingBottom: '10px' };
